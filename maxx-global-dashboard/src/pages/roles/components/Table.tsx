// src/pages/roles/components/Table.tsx
import { useEffect, useRef, useState } from "react";
import type { RoleRow } from "../../../types/role";

export type PageLocal<T> = {
  content: T[];
...
type Props = {
  data: PageLocal<RoleRow>;
  onEdit: (r: RoleRow) => void;
  onAskDelete: (r: RoleRow) => void;
};

/** Küçük bir popover: hover veya tıkla ile açılır, dışarı tıklayınca kapanır */
function PermissionPreviewBadge({
  items,
}: {
  ...
  const onLeave = () => {
    // küçük bir gecikme, mouse popover içine geçerken kapanmasın
  ...
  return (
    ...
    >
      <button
        ...
        title={count ? "Yetkileri gör" : "Yetki yok"}
      >
        {count} yetki
        {count ? (
          <svg
...
export default function RolesTable({ data, onEdit, onAskDelete }: Props) {
    ...
    <div className="sherah-table p-0">
      <table className="sherah-table__main sherah-table__main-v3">
        <thead className="sherah-table__head">
          <tr>
            <th className="sherah-table__column-1 sherah-table__h1">Rol Adı</th>
            ...
            <th className="sherah-table__column-4 sherah-table__h4">Durum</th>
            <th className="sherah-table__column-5 sherah-table__h5">Aksiyon</th>
          </tr>
        </thead>
              ...
              <tr key={r.id}>
                <td className="sherah-table__column-1 sherah-table__data-1">
                  <div className="sherah-table__product-content">
                    <p className="sherah-table__product-desc">{r.name}</p>
                  </div>
                </td>
                ...
                <td className="sherah-table__column-2 sherah-table__data-2">
                  <div className="sherah-table__product-content">
                    {(r.permissions?.length ?? 0) > 0 ? (
                      <PermissionPreviewBadge items={r.permissions!} />
                    ) : (
                      <span className="text-muted  bg-danger">
