...
// ✅ İndirim tipini kontrol eden yardımcı fonksiyon
function isPercentageDiscount(discountType: any): boolean {
  // Backend'den gelen enum değerlerini kontrol et
  console.log("Checking discountType:", discountType, "type:", typeof discountType);
  
  // Eğer object olarak geliyorsa (enum object)
  ...
  const result = typeStr === "PERCENTAGE" || 
         typeStr === "Yüzdesel" || 
         typeStr === "PERCENT" ||
...
export default function FlashSale() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!dealerId) {
          setError("Bayi bilgisi bulunamadı. Lütfen giriş yapmayı kontrol edin.");
          return;
        }

        // ✅ Sadece discount listesini çek, image loading'i kaldır
        const discountList = await listDiscountsByDealer(dealerId);
        
        // ✅ Aktif ve geçerli indirimleri filtrele
        const activeDiscounts = discountList.filter(d => 
          d.isActive && d.isValidNow
        );

        // ✅ DEBUG - Console'da discountType değerlerini kontrol edelim
        console.log("Active discounts with types:", activeDiscounts.map(d => ({
          name: d.name,
  ...
  // ✅ Aynı isme sahip indirimleri grupla
          ...
          {!error && grouped.length > 0 && (
            <div className="grid lg:grid-cols-1 gap-6">
              ...
              {grouped.map((discount) => (
                <div key={discount.id} className="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-lg transition-all duration-300">
                  {/* Card Header */}
                  <div className="p-6 border-b border-gray-100">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-4">
                        {/* ✅ İndirim badge - Düzeltildi */}
                        <div className={`${getDiscountBadgeColor(discount)} text-white px-4 py-2 rounded-full text-lg font-bold`}>
                          {(() => {
                            const isPercentage = isPercentageDiscount(discount.discountType);
                            console.log("Badge rendering - discount:", discount.name, "type:", discount.discountType, "isPercentage:", isPercentage, "value:", discount.discountValue);
                        ...
                        <div>
                          ...
                          <p className="text-gray-600 mt-1">{discount.description || "Özel indirim kampanyası"}</p>
                        </div>
                      </div>
                      ...
                      <div className="text-right">
                        <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
                          getDaysLeft(discount.endDate).includes("dolmuş") 
                            ...
                            : getDaysLeft(discount.endDate).includes("1 gün") || getDaysLeft(discount.endDate).includes("2 gün") || getDaysLeft(discount.endDate).includes("3 gün")
                  ...
                  <div className="p-6">
                    <div className="grid md:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                          <div className="bg-gray-50 rounded-lg p-4">
                            ...
                            <div className="text-sm text-gray-600">Geçerli Ürün</div>
                          </div>
                          <div className="bg-gray-50 rounded-lg p-4">
                            <div className="text-2xl font-bold text-gray-900">
                              {discount.allDealers?.length || 0}
                            </div>
                            <div className="text-sm text-gray-600">Geçerli Bayi</div>
                          </div>
                        </div>
                          ...
                          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                            <div className="flex items-center">
                              ...
                              <span className="text-sm text-blue-700">
                                Minimum sipariş tutarı: <strong>{fmt(discount.minimumOrderAmount, dealerCurrency)}</strong>
                              </span>
                            </div>
                          </div>
                        )}
                          ...
                          <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
                            <div className="flex items-center">
                              ...
                              <span className="text-sm text-purple-700">
                                Maksimum indirim: <strong>{fmt(discount.maximumDiscountAmount, dealerCurrency)}</strong>
                              </span>
                            </div>
                          </div>
                        )}
                      ...
                      {/* Sağ taraf - Tarih bilgileri ve popover'lar */}
                      <div className="space-y-4">
                        <div className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-lg p-4">
                          <h4 className="font-semibold text-gray-900 mb-3">Kampanya Tarihleri</h4>
                        ...
                        <div className="flex flex-col space-y-3">
                          {discount.allProducts?.length > 0 && (
                            <div className="flex items-center justify-between">
                              ...
                              <PopoverBadge products={discount.allProducts} dealers={[]} />
                            </div>
                          )}
                          
                          {discount.allDealers?.length > 0 && (
                            <div className="flex items-center justify-between">
                              <span className="text-sm font-medium text-gray-700">Bayiler:</span>
                              <PopoverBadge products={[]} dealers={discount.allDealers} />
                            </div>
                          )}
                        </div>
                          ...
                          <div className="bg-gray-50 rounded-lg p-4">
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <span className="text-xs text-gray-500">
                                {discount.totalUsage || 0} / {discount.usageLimit}
                              </span>
                            </div>
                            <div className="w-full bg-gray-200 rounded-full h-2">
                              <div 
                                ...
                                style={{ 
                                  width: `${Math.min(100, ((discount.totalUsage || 0) / discount.usageLimit) * 100)}%` 
                                }}
                              ></div>
                            </div>
                          </div>
          ...
          {!error && grouped.length === 0 && (
            <div className="text-center py-12">
              <div className="bg-gray-50 rounded-lg p-8 max-w-md mx-auto">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">Henüz indirim bulunmuyor</h3>
                <p className="text-gray-600">Bu bayi için aktif indirim kampanyası bulunmamaktadır.</p>
              </div>
            </div>
          )}
        </div>
